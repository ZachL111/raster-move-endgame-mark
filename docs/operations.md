...
`raster-move-endgame-mark` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies endgame behavior through transition tables, with invalid-transition tests and offline replay mode.
...
The score is deterministic. The current threshold is 184. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 3.
